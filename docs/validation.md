...
## 2026-09-22 — Center the first presentation after startup

The old show path queued WindowCenter before WindowShow. GTK documents that a window manager may ignore initial placement and honor moves after mapping; the existing frontend readiness handshake only guarded styling, not placement. Added a GTK map-event handler to reapply centering using allocated native dimensions and current monitor geometry. The first presentation keeps native opacity zero until the observed root coordinates match the center (within one logical pixel), with an 8 ms check and a bounded one-second fallback for window managers that ignore absolute positioning. Hide/shutdown invalidates pending presentation callbacks. Warm openings keep the existing motion durations and no cold-presentation wait.

Tagged go vet and production Wails/TypeScript build passed. Extended native-smoke.py with independent X11 geometry reads filtered to the isolated test binary. Passed 13 geometry observations including the first show of a background process, subsequent focus cycles and a fresh shortcut-start process. Also passed guarded startup, 10 focus cycles, 100 rapid toggles, stale-hide cancellation and clean shutdown. Frontend ready 547.9 ms; sampled hide latency 201.0 ms. Tested in the running Cinnamon/X11 session at 1920×1080; a full logout/reboot and other window managers were not exercised.

Reference: https://docs.gtk.org/gtk3/method.Window.move.html and https://docs.gtk.org/gtk3/signal.Widget.map-event.html .
